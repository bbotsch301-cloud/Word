...
function getUserDatabase() {
  ...
  try {
    ...
    const dbPath = path.join(process.cwd(), "data", "user.db");
...
export function createUser(email: string, name?: string, passwordHash?: string): User {
  const db = getUserDatabase();
  if (!db) throw new Error("User database unavailable");
...
export function verifyListOwnership(listId: string, userId: string): boolean {
  const db = getUserDatabase();
  if (!db) return false;
  const row = db.prepare(
